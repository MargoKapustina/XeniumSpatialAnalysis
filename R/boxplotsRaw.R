...
XeniumBoxPlotRaw = function(genes, ncol = 4, object, save_plot = FALSE) {
  ...
  suppressWarnings({ #supresses following warning:
    ...
    Plot = function(theGenes) {
      VlnPlot(object, features = theGenes, pt.size = 0, layer = "counts", adjust = 0) +
        theme(legend.position = "nolegend") +
        geom_boxplot(outlier.size = 0, outlier.stroke = 0, lwd = 0.3) +
        theme(axis.line = element_line(linewidth = 0.5), axis.text.y = element_text(size = 6), axis.text.x = element_blank(),
