...
getUMAP1_MidlineData <- function(object, FOV, degs = 45, binNumber = 15, binwidth = 40) {
  ...
  gg <- gg + ggplot2::geom_point(aes(colour= UMAP_1), size = 1.5)
  gg <- gg + ggplot2::geom_abline(slope=m)
  gg <- gg + ggplot2::coord_fixed() +ggtitle('UMAP_1 values across Spatial Midline')+scale_color_viridis_c()
  print(gg)
  ...
  p1 = ggplot2::ggplot(myplot,aes(x = dLine)) +
    ggplot2::geom_histogram(aes(y = stat(count / 100)), bins = binNumber, boundary= 0, colour = 'green')+
    coord_cartesian(xlim = c(-500, 500), ylim =c(-6,8))+
    xlab('Distance away from Spatial Midline')+
    ylab('Cells per bin/100')+
    stat_summary_bin(aes(x = dLine, y= UMAP_1),fun = mean, binwidth = binwidth, geom = 'line', colour = 'blue') +
    #stat_bin(bins = 7, boundary = 0,geom = 'text', aes(label = ..count../100), vjust = 0, size = 3, color = 'black')+
    ggtitle('UMAP_1 embeddings\nbins are mean number of cells\nper bin /100 in this FOV ONLY')
  print(p1|gg)
...
plotUMAP1_vsMidline <- function(UMAP1MidlineData,
  ...
  # ggplot2 Visualization
  #show UMAP_1 embeddings as a line graph, as a distance away from midline
  p1 = ggplot(pooled_UMAP_df,aes(x = dLine)) +
    geom_histogram(aes(y = stat(count / 100)), bins = binNumber, boundary= 0, colour = 'green')+
    coord_cartesian(xlim = c(-500, 500), ylim =c(-6,8))+
    xlab('Distance away from Spatial Midline')+
    ylab('Cells per bin/100')+
    stat_summary_bin(aes(x = dLine, y= UMAP_1),fun = mean, binwidth =binwidth, geom = 'line', colour = 'blue') +
    #stat_bin(bins = 7, boundary = 0,geom = 'text', aes(label = ..count../100), vjust = 0, size = 3, color = 'black')+
    ggtitle('UMAP_1 embeddings across Spatial Midline')
  print(p1)
